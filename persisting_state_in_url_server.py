...
the url and query params to `persist state` to increment/decrement/display a
...
def get_response_body(request_line: str, http_method: str,
    ...
    number = int(params_dct['number'])
    return (
        "<html><head><title>URL `Stateful` Counter</title></head><body>"
        ...
        "<h2>Counter:</h2>"
        f"<p style='color: red;'>The curr number is {number}</p>"
